...
past = list.files(path = 'data/microclim_3D/mosaics/mosaics_meanTmax/mean_Tmax_20m/past/', pattern = '.*past_.*.tif', full.names = T)
pres = list.files(pat = 'data/microclim_3D/mosaics/mosaics_meanTmax/mean_Tmax_20m/pres/', pattern = '.*pres_.*.tif', full.names = T)
...
# min temp (bio6)
past = list.files(path = 'data/microclim_3D/mosaics/mosaics_temp_bio6/temp_bio6_20m/past/', pattern = '.*past_.*.tif', full.names = T)
pres = list.files(pat = 'data/microclim_3D/mosaics/mosaics_temp_bio6/temp_bio6_20m/pres', pattern = '.*pres_.*.tif', full.names = T)
heights = c(2,5,10,15,20,25,30,35,40)
...
# max temp of the warmest month (bio5)
past = list.files(path = 'data/microclim_3D/mosaics/mosaics_temp_bio5/temp_bio5_20m/past', pattern = '.*past_.*.tif', full.names = T)
pres = list.files(pat = 'data/microclim_3D/mosaics/mosaics_temp_bio5/temp_bio5_20m/pres', pattern = '.*pres_.*.tif', full.names = T)
heights = c(2,5,10,15,20,25,30,35,40)
...
foreach (i=c(1:length(past)), .packages=c('terra', 'data.table', 'numform')) %dopar% {
  if (i <= length(past)-2) {
    ...
    fname_sp = paste0('output/3D/bio5/spatialGradient_', mid, 'm.tif')
    fname_tg = paste0('output/3D/bio5/temporalGradient_', mid, 'm.tif')
    fname_rast = paste0('output/3D/bio5/vocc_', mid, 'm.tif')
    fname_csv = paste0('output/3D/bio5/vocc_', mid, 'm.csv')
