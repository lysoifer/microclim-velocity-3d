...
pai.micro2d.1km = resample(pai.micro, dem.micro2d.1km)
...
# micro2d 20m resolution --------------------------------------------------

# combine into raster for each scale
micro2d = c(micro2d$vocc, micro2d$magnitude, micro2d$xyAng, micro2d$NSdir, micro2d$EWdir,
            tempgrad.micro2d, dem.micro, aspect.micro, pai.micro, micro.paidir,
            landuse.micro, maxtemp.pres.micro/100)
names(micro2d) = c("vocc", "spatgrad", "xyAng", "NSdir", "EWdir", "tempgrad", 
                   "elev", "aspect", "pai", "paidir", "landuse", "maxtemp.pres")
micro2d = micro2d %>% 
  as.data.frame(xy = T) %>% 
...
micro2d.100m = c(micro2d.100m$vocc, micro2d.100m$magnitude, micro2d.100m$xyAng, micro2d.100m$NSdir, micro2d.100m$EWdir,
            tempgrad.micro2d.100m, dem.meso, aspect.meso, pai.meso, meso.paidir,
            landuse.meso, maxtemp.pres.micro.100m)
names(micro2d.100m) = c("vocc", "spatgrad", "xyAng", "NSdir", "EWdir",
                        "tempgrad", "elev", "aspect", "pai", "paidir",
                        "landuse", "maxtemp.pres")
...
# micro2d 1km -------------------------------------------------------------
micro2d.1km = c(micro2d.1km$vocc, micro2d.1km$magnitude, micro2d.1km$xyAng, 
                micro2d.1km$NSdir, micro2d.1km$EWdir,
                tempgrad.micro2d.1km, dem.micro2d.1km, aspect.micro2d.1km, 
                pai.micro2d.1km, micro2d.1km.paidir, 
                landuse.micro2d.1km, maxtemp.pres.micro.1km)

names(micro2d.1km) = c("vocc", "spatgrad", "xyAng", "NSdir", "EWdir", "tempgrad", 
                       "elev", "aspect", "pai", "paidir",
                       "landuse", "maxtemp.pres")
...
# free-air 100 m resolution -----------------------------------------------

meso = c(meso$vocc, meso$magnitude, meso$xyAng, meso$NSdir, meso$EWdir,
         tempgrad.meso, dem.meso, aspect.meso, pai.meso, meso.paidir,
         landuse.meso, maxtemp.pres.meso)
...
# free-air 1km resolution -------------------------------------------------
macro = c(macro$vocc, macro$magnitude, macro$xyAng, macro$NSdir, macro$EWdir,
          tempgrad.macro, dem.macro, aspect.macro, pai.macro, macro.paidir,
          landuse.macro, maxtemp.pres.macro)
names(macro) = c("vocc", "spatgrad", "xyAng", "NSdir", "EWdir", "tempgrad", 
                 "elev", "aspect", "pai", "paidir",
                 "landuse", "maxtemp.pres")
macro = macro %>% 
  as.data.frame(xy = T) %>% 
...
# micro2d 20 m resolution ------------------------------------------------------------
micro2d = c(micro2d$vocc, micro2d$magnitude, micro2d$xyAng, micro2d$NSdir, micro2d$EWdir,
            tempgrad.micro2d, dem.micro, aspect.micro,
            pai.micro, micro.paidir,
            landuse.micro, mintemp.pres.micro/100)
names(micro2d) = c("vocc", "spatgrad", "xyAng", "NSdir", "EWdir", "tempgrad",
                   "elev", "aspect", "pai", "paidir", "landuse", "mintemp.pres")
micro2d = micro2d %>% 
  as.data.frame(xy = T) %>% 
  mutate(vocc = ifelse(vocc == Inf, NA, vocc),
         vocc = abs(vocc)) %>% 
...
micro2d.100m = c(micro2d.100m$vocc, micro2d.100m$magnitude, micro2d.100m$xyAng,
                 micro2d.100m$NSdir, micro2d.100m$EWdir,
                tempgrad.micro2d.100m,
                dem.meso, aspect.meso,
                pai.meso, meso.paidir,
                landuse.meso, mintemp.pres.micro.100m)
names(micro2d.100m) = c("vocc", "spatgrad", "xyAng", "NSdir", "EWdir", "tempgrad", 
                        "elev", "aspect", "pai", "paidir", "landuse", "mintemp.pres")
micro2d.100m = micro2d.100m %>% 
  as.data.frame(xy = T) %>% 
  mutate(vocc = ifelse(vocc == Inf, NA, vocc),
...
micro2d.1km = c(micro2d.1km$vocc, micro2d.1km$magnitude, micro2d.1km$xyAng, 
                micro2d.1km$NSdir, micro2d.1km$EWdir,
                 tempgrad.micro2d.1km, 
                dem.micro2d.1km, aspect.micro2d.1km,
                pai.micro2d.1km, micro2d.1km.paidir,
                landuse.micro2d.1km, mintemp.pres.micro.1km)
names(micro2d.1km) = c("vocc", "spatgrad", "xyAng", "NSdir", "EWdir", "tempgrad", 
                       "elev", "aspect", "pai", "paidir", "landuse", "mintemp.pres")
micro2d.1km = micro2d.1km %>% 
  as.data.frame(xy = T) %>% 
...
meso = c(meso$vocc, meso$magnitude, meso$xyAng, meso$NSdir, meso$EWdir,
         tempgrad.meso, dem.meso, aspect.meso, pai.meso, meso.paidir,
         landuse.meso, mintemp.pres.meso)
names(meso) = c("vocc", "spatgrad", "xyAng", "NSdir", "EWdir", 
                "tempgrad", "elev", "aspect", "pai", "paidir", "landuse", "mintemp.pres")
meso = meso %>% 
  as.data.frame(xy = T) %>% 
  mutate(vocc = ifelse(vocc == Inf, NA, vocc),
         vocc = abs(vocc)) %>% 
...
macro = c(macro$vocc, macro$magnitude, macro$xyAng, macro$NSdir, macro$EWdir,
          tempgrad.macro, dem.macro, aspect.macro,
          pai.macro, macro.paidir, landuse.macro, mintemp.pres.macro)
names(macro) = c("vocc", "spatgrad", "xyAng", "NSdir", "EWdir", "tempgrad", 
                 "elev", "aspect", "pai", "paidir", "landuse", "mintemp.pres")
macro = macro %>% 
  as.data.frame(xy = T) %>% 
...
mintemp3d = fread("data/dataframes/analysis_dataframe_3d_full_mintemp.csv")
df_mintemp = bind_rows(macro, meso, micro2d, micro2d.100m, micro2d.1km, mintemp3d)
